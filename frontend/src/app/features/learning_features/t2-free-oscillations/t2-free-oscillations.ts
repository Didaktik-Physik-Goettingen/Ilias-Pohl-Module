...
export class T2FreeOscillations implements OnInit, AfterViewInit, OnDestroy {
  ...
  currentView = 't2_page1';
  get isFirstPage(): boolean { return this.currentView === 't2_page1'; }
  get isLastPage(): boolean { return this.currentView === 't2_page3'; }
  ...
  goBack(): void {
    if (this.currentView === 't2_page1') {
      this.router.navigate(['/learning/t1-intro-experiment']); return;
    }
    const n = parseInt(this.currentView.replace('t2_page', ''));
    this.currentView = `t2_page${n - 1}`;
    this.updateUrl(); this.renderMath();
  }

  goForward(): void {
    if (this.currentView === 't2_page1' && !this.isCorrect1) return;
    if (this.currentView === 't2_page2' && (!this.isCorrect4 || !this.isCorrect5 || !this.isCorrect6)) return;
    if (this.currentView === 't2_page3') {
  ...
  private updateUrl(): void {
    const page = this.currentView.replace('t2_page', '');
