...
@Component({
    selector: 'app-sim-t-driven-x-oscillations',
    imports: [FormsModule],
    templateUrl: './sim-t-driven-x-oscillations.html',
    styleUrl: './sim-t-driven-x-oscillations.css',
    host: { ngSkipHydration: 'true' },
})
export class SimTDrivenXOscillations implements OnInit, AfterViewInit, OnDestroy {
    ...
    onToggleCanvas(which: 'time' | 'phase') {
        this.interactionCounter();
        if (which === 'time'  && this.showTimeDiagram) this.drawAxes(this.ctxTime,  this.canTime,  't',    'x(t)', this.state.dx,             this.canTime.height  * 0.5);
        if (which === 'phase' && this.showPhase)       this.drawAxes(this.ctxPhase, this.canPhase, 'x(t)', 'ẋ(t)', this.canPhase.width * 0.5, this.canPhase.height * 0.5);
    }

    onMotorToggle()      { if (!this.dynamicMode) { this.reset(); this.drawFunctions(); } }
    ...
    private reset() {
        ...
        if (this.showTimeDiagram) this.drawAxes(this.ctxTime,  this.canTime,  't',    'x(t)', this.state.dx,             this.canTime.height  * 0.5);
    ...
    private computePhysics(t: number) {
        ...
        return { x, x_hom, x_inhom, v, xmotor };
    }
    ...
    private readonly step = () => {
        ...
        if (this.showPhase) {
            this.drawAxes(this.ctxPhase, this.canPhase, 'x(t)', 'ẋ(t)', this.canPhase.width * 0.5, this.canPhase.height * 0.5);
            this.state.pointsPhase.push({ x: this.canPhase.width * 0.5 + phys.x, y: this.canPhase.height * 0.5 + phys.v, alpha: 1 });
    ...
    private drawFunctions() {
        this.interactionCounter();
        const pts: Point[] = [], ptsM: Point[] = [], ptsH: Point[] = [], ptsI: Point[] = [], ptsPh: Point[] = [];
        let t = 0;
        while (this.state.dx + 10 * t < 0.94 * this.canTime.width) {
            const phys = this.computePhysics(t);
            const xPix = this.state.dx + 10 * t;
            pts.push(  { x: xPix, y: this.canTime.height * 0.5 - phys.x,       alpha: 1   });
            ptsM.push( { x: xPix, y: this.canTime.height * 0.5 - phys.xmotor,  alpha: 0.5 });
            ptsH.push( { x: xPix, y: this.canTime.height * 0.5 - phys.x_hom,   alpha: 0.5 });
            ptsI.push( { x: xPix, y: this.canTime.height * 0.5 - phys.x_inhom, alpha: 0.5 });
            if (this.showPhase) ptsPh.push({ x: this.canPhase.width * 0.5 + phys.x, y: this.canPhase.height * 0.5 + phys.v, alpha: 1 });
            t += this.state.dt;
        }
        if (this.showTimeDiagram) {
            this.drawAxes(this.ctxTime, this.canTime, 't', 'x(t)', this.state.dx, this.canTime.height * 0.5);
            if (this.showMotor) { this.drawTrail(this.ctxTime, ptsM, '#ff6666'); }
            else { this.drawTrail(this.ctxTime, ptsH, 'green'); this.drawTrail(this.ctxTime, ptsI, 'blue'); }
