...
// explanations) — see src/app/core/services/report-mode.ts. It is dev-only, so
// it works here (ng serve) but is ignored in production builds.
...
// `base` in sync with TEMPLATE_MAP in api/report.php, and `pages` with each
// route's subpage count.
const TARGETS = [
